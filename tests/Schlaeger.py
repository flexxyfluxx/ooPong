...
params:
> container: Grid-Fenster, in dem sich der Schläger befindet.
> key_up: Tastencode der Taste, die den Schläger nach oben bewegt.
> key_dn: Tastencode der Taste, die den Schläger nach unten bewegt.
> max_y: Untere y-Bande für den Schläger. (Obere Bande liegt immer bei y=0)
"""
class Schlaeger(gg.Actor):
    def __init__(self, container, key_up, key_dn, max_y = WINDOW_HEIGHT): # Wird kein max_y-Wert gegeben, ist der Standard, dass der untere Rand gerade berührt wird.
    ...
    def move(self):
        ...
        if self._last_direction == NORTH:
            if self.container.isKeyPressed(self.key_dn):
                self.setY(self.getY() + PADDLE_SPEED)
                self._last_direction = SOUTH
                
            elif self.container.isKeyPressed(self.key_up):
                self.setY(self.getY() - PADDLE_SPEED)
                
        elif self._last_direction == SOUTH:
            if self.container.isKeyPressed(self.key_up):
                self.setY(self.getY() - PADDLE_SPEED)
                self._last_direction = NORTH
                
            elif self.container.isKeyPressed(self.key_dn):
                self.setY(self.getY() + PADDLE_SPEED)
